...
function HomePage({ groups, setPage }) {
  return (
    <div>
          ...
          groups.map((group) => {
            ...
            return (
              ...
              >
                ...
                <p>👥 {total} participant{total !== 1 ? 's' : ''}: {preview}{showEllipsis && '...'}</p>
                <p>💸 {group.expenses.length} expense{group.expenses.length !== 1 ? 's' : ''}</p>
              </div>
            );
          })
        )}
      ...
      <button className="button" onClick={() => setPage({ name: 'add' })}>➕ Add Group</button>
    </div>
  );
}
